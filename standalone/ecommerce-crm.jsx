...
const NON_UPLINE_ROLES = ["夥伴", ...RECRUIT_ROLES];
...
const SEED_INTERACTIONS = [
  { id: "i1", date: "2025-03-24", partnerId: "p2", type: "討論", title: "Q2 業績規劃", content: "討論了四月份的推廣策略。", status: "已完成", tags: ["業績"] },
  ...
  { id: "i5", date: "2025-03-30", partnerId: "p4", type: "討論", title: "談場前資料準備", content: "確認見面時間地點，準備產品介紹資料。", status: "待執行", tags: ["招募"] },
];
const SEED_TODOS = [
...
const css = `
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .type-討論{color:var(--blue)}.type-追蹤{color:var(--gold)}.type-規劃{color:var(--green)}.type-上線會議{color:var(--purple)}
  .tl-dot.type-討論{background:var(--blue)}.tl-dot.type-追蹤{background:var(--gold)}.tl-dot.type-規劃{background:var(--green)}.tl-dot.type-上線會議{background:var(--purple)}
  ...
  .cal-event.type-討論{background:#eff6ff;color:var(--blue)}
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      setPartners((await load(KEYS.partners)) || SEED_PARTNERS);
      setInteractions((await load(KEYS.interactions)) || SEED_INTERACTIONS);
    ...
    <>
      ...
      <div className="app">
        ...
        <main className="main">
          ...
          {tab==="partners"  && <Partners  partners={partners} setPartners={p=>persist(KEYS.partners,p,setPartners)} interactions={interactions} rawSave={p=>save(KEYS.partners,p)}/>}
...
// ─── Dashboard ────────────────────────────────────────────────────
function Dashboard({ partners, interactions, setInteractions, todos, goals, setGoals, setTodos, manifest, setManifest, incomes, persistIncomes }) {
...
// ─── Partners ─────────────────────────────────────────────────────
function Partners({ partners, setPartners, interactions, rawSave }) {
  ...
  const saveP = () => {
    const entry={...editData,tags:editData.tags?editData.tags.split(/[、,，]/).map(t=>t.trim()).filter(Boolean):[],avatar:editData.avatar||editData.name[0]||"?"};
  ...
  const partnerInteractions = selected?interactions.filter(i=>i.partnerId===selected.id).sort((a,b)=>b.date.localeCompare(a.date)):[];

  const roleBadge = (role) => {
    if(role==="夥伴") return <span className="role-badge role-夥伴">{role}</span>;
      ...
      {selected&&(
        <Modal title="" onClose={()=>setSelected(null)} wide>
          <div className="flex items-center gap-12 mb-14">
            <div className="avatar avatar-lg">{selected.avatar||selected.name[0]}</div>
            <div>
          ...
          {detailTab==="log"&&(
            <div>
              <div className="subheading mb-8">互動紀錄 ({partnerInteractions.length})</div>
...
function Timeline({ interactions, setInteractions, partners }) {
  ...
  const openNew = () => { setEditData({id:uid(),date:new Date().toISOString().slice(0,10),partnerId:"",type:"討論",title:"",content:"",status:"待執行",tags:"",partnerPlan:"",actionItems:"",quote:""}); setShowForm(true); };
  const saveItem = () => {
    ...
    // auto-create action items as separate 規劃 entries
    if(editData.type==="上線會議" && editData.actionItems?.trim()){
      const lines=editData.actionItems.split("\n").map(l=>l.trim()).filter(Boolean);
  ...
  const del = (id) => { setInteractions(interactions.filter(i=>i.id!==id)); setSelected(null); };
    ...
    <div>
      ...
      {view==="list"&&(
        <>
          <div className="flex gap-8 mb-14" style={{flexWrap:"wrap"}}>
            {["全部","待執行","上線會議","討論","追蹤","規劃"].map(f=><button key={f} className={`btn btn-sm ${filter===f?"btn-gold":"btn-ghost"}`} style={f==="上線會議"&&filter!==f?{borderColor:"#c4b5fd",color:"var(--purple)"}:{}} onClick={()=>setFilter(f)}>{f}</button>)}
          </div>
            ...
            {calDays().map((cell,i)=>{
              ...
              return (
                <div key={i} className={`cal-cell${k===today?" today":""}${!cell.cur?" other-month":""}`}>
                  ...
                  {dayItems.length>3&&<div style={{fontSize:8,color:"var(--text3)"}}>+{dayItems.length-3}</div>}
                </div>
              );
            })}
          </div>
      ...
      {selected&&(
        <Modal title={selected.title} onClose={()=>setSelected(null)}>
          ...
          {selected.type==="上線會議"&&selected.partnerPlan&&(
            <div className="card-sm mt-10"><div className="label">針對夥伴的具體規劃</div><div className="text-sm mt-5" style={{lineHeight:1.8}}>{selected.partnerPlan}</div></div>
          )}
          ...
          <div className="flex gap-8">
            <button className="btn btn-gold btn-sm" onClick={()=>{ setSelected(null); setEditData({...selected,tags:Array.isArray(selected.tags)?selected.tags.join("、"):selected.tags,partnerPlan:selected.partnerPlan||"",actionItems:selected.actionItems||"",quote:selected.quote||""}); setShowForm(true); }}>編輯</button>
            <button className="btn btn-danger btn-sm" onClick={()=>del(selected.id)}>刪除</button>
          </div>
        </Modal>
      )}
        ...
        <Modal title={`${interactions.find(i=>i.id===editData.id)?"編輯":"新增"}紀錄`} onClose={()=>setShowForm(false)} wide>
          <div className="form-row">
            ...
            <div className="form-group"><label className="label">類型</label>
              <select className="input" value={editData.type} onChange={e=>setEditData({...editData,type:e.target.value,partnerId:e.target.value==="上線會議"?"":editData.partnerId})}>
                {["討論","追蹤","規劃","上線會議"].map(t=><option key={t}>{t}</option>)}
              </select>
            </div>
          </div>
          ...
          {editData.type==="上線會議"?(
            <div style={{background:"#f5f3ff",border:"1.5px solid #c4b5fd",borderRadius:10,padding:"14px 16px",marginBottom:12}}>
              ...
              <div className="form-group"><label className="label">針對夥伴的具體規劃</label><textarea className="input" style={{minHeight:60}} value={editData.partnerPlan||""} onChange={e=>setEditData({...editData,partnerPlan:e.target.value})} placeholder="例：要在本週跟王思涵約談場、幫陳威宇準備產品資料…"/></div>
              <div className="form-group"><label className="label">待執行行動項目（一行一項，會自動加入待執行清單）</label><textarea className="input" style={{minHeight:72,fontFamily:"'DM Mono',monospace",fontSize:12}} value={editData.actionItems||""} onChange={e=>setEditData({...editData,actionItems:e.target.value})} placeholder={"傳產品資料給王思涵\n幫陳威宇整理獎金說明\n本週跟張育豪確認談場時間"}/></div>
